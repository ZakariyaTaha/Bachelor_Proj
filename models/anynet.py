    ...
    def forward(self, left, right):
        ...
        for scale in range(len(feats_l)):
            ...
            if scale == 0:
                ...
                pred_low_res = disparityregression2(-self.maxdisplist[scale] + 1, self.maxdisplist[scale], stride=1)(
        ...
        if self.refine_spn:
            ...
            G1, G2, G3 = spn_out[:, :self.spn_init_channels, :, :], spn_out[:,
                                                                    self.spn_init_channels:self.spn_init_channels * 2,
                                                                    :, :], spn_out[:, self.spn_init_channels * 2:, :, :]
